        ...
        <tr class="coupon_row" id="coupon_row_{{ $coupon->id }}">
            ...
            <td>
                <div class="form-check form-switch form-switch-sm mb-3" dir="ltr">
                    <input class="form-check-input switchCouponStatus" type="checkbox" data-id="{{ $coupon->id }}" id="switchCouponStatus_{{ $coupon->id }}" {{ ($coupon->status == 1)?'checked':'' }}>
                </div>
            </td>
            @canany(['update','delete'], $coupon)
                <td>
                    <div class="d-flex gap-3">
                        @can('update', $coupon)
                            <a href="javascript:void(0);" class="text-success edit_coupon" data-id="{{ $coupon->id }}" data-bs-toggle="modal" data-bs-target="#editCoupon">
                                <i class="mdi mdi-pencil font-size-18"></i>
                            </a>
                        @endcan
                        @can('delete', $coupon)
                            <a href="javascript:void(0);" class="text-danger coupon_delete" data-id="{{ $coupon->id }}" data-bs-toggle="modal" data-bs-target="#deleteCoupon">
